...
#         gain = torch.ones(7 + nc, device=targets.device).long()
#         ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
...
def compute_distillation_output_loss(p, t_p, model, dist_loss="l2", T=20, reg_norm=None):
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        # Adjust t_pi to match pi shape
        if t_pi.shape[-1] > pi.shape[-1]:
            t_pi = t_pi[..., :pi.shape[-1]]  # truncate t_pi to match pi shape
        elif t_pi.shape[-1] < pi.shape[-1]:
            padding = pi.shape[-1] - t_pi.shape[-1]
            t_pi = torch.nn.functional.pad(t_pi, (0, padding))  # pad t_pi to match pi shape
...
class ComputeDstillLoss:
    ...
    def __call__(self, p, targets, soft_loss='kl'):
        device = targets.device
        dloss = torch.zeros(1, device=device)
        tcls, tbox, tlogits, indices, anchors = self.build_targets(p, targets)
